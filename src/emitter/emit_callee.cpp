...
std::string emit_function_callee(std::shared_ptr<function_information> f, size_t indent) {
	...
	if (f -> attr_flag & ATTRIBUTE_INVERSE) {
		script = 
			"int __wrapper_$(NAME)(void* input, void** output) {\n"
			"$(ALLOCATE_ARGUMENTS)"
			"\t__ocall_wrapper_$(NAME)_table_t function_reference = __ocall_wrapper_$(NAME)_as_root(input);\n"
